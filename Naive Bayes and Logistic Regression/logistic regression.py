...
def cross_val(groups1: list, groups2: list, verbal_fluencies: list, C: float):
...
def simple_train_test(groups1: list, groups2: list, verbal_fluencies: list, C: float):
...
def merged_cross_val(merged_groups, verbal_fluencies, C: float):
...
def gridsearch_crossval(groups1: list, groups2: list, verbal_fluencies: list):
    ...
    results = pd.DataFrame(columns=['Groups', 'VF', 'acc', 'F1', 'MCC', 'penalty', 'C', 'best cv'])
    ...
    for (i, (vf, g1, g2)) in enumerate(iterate_list):
        ...
        grid_search = GridSearchCV(logreg(solver='liblinear'), param_grid, cv=5, return_train_score=True)
        # Fit to training data
        model = grid_search.fit(X_train, y_train)

        # Extracting parameters and kernel and best scores
        best_params = dict.fromkeys(['C', 'penalty'])
        ...
        results.loc[i] = [g1 + ' & ' + g2, vf, test_score, F1, MCC, best_penalty, best_C, best_cv]

    results.to_csv('LogisticRegression_GSCV_results.csv', index=False)
    print(results)
...
if __name__ == '__main__':
    ...
    #C = 1.0
    # merged_cross_val(merged_groups=groupnames[1:], verbal_fluencies=vfs, C=C)
    #simple_train_test(groups1=['control'], groups2=groupnames, verbal_fluencies=['flib'], C=C)
    improve_dataframe_display()
    # cross_val(groups1=groupnames, groups2=groupnames, verbal_fluencies=['flib'], C=C)
    # gridsearch_crossval(groups1=groupnames, groups2=groupnames, verbal_fluencies=vfs)
    df = pd.read_csv('LogisticRegression_GSCV_results.csv')
    select = df[df['Groups'] == 'mania & depression']
    #select = df.sort_values(by='Groups')
    #select = select[select['acc'] >= 0.70]
    #select.to_csv('LogReg_mania_&_mixed_mania.csv')
    print(select)
