...
for (j, feature) in enumerate(anchor_cols):
    ...
    importance_mean = pd.DataFrame(columns=['task'] + simple_cols)
    importance_std = pd.DataFrame(columns=['task'] + simple_cols)
    for (i, (task, cols)) in enumerate(task_features.items()):
        ...
        importance_mean.loc[i] = [task] + list(perm_importance.importances_mean)
        importance_std.loc[i] = [task] + list(perm_importance.importances_std)
        print(task)
